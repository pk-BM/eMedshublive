...
const News = () => {
  const [newsList, setNewsList] = useState([]);  const [loading, setLoading] = useState(false);

  useEffect(() => {    const fetchNews = async () => {

  try {
        setLoading(true);
        const response = await GetAllNews();  const fetched = response?.data || [];

        // Sort latest first
        const sortedNews = fetched.sort(          (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
        );
         // Map required fields
  ...
  return (
    <div className="flex flex-col md:flex-row justify-center gap-8 px-6 md:px-28">
      ...
      >
        ...
        </div>    <ul className="p-4 space-y-3">
