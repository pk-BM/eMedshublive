...
import { GetAllDoctorAdvices } from "../../../lib/APIs/doctorAdviceAPI";

const News = () => {
  const [newsList, setNewsList] = useState([]);
  const [doctorAdviceList, setDoctorAdviceList] = useState([]);
  ...
  const fetchNews = async () => {
    try {
      ...
      const filledNews = [
        ...formattedNews,
        ...Array(Math.max(0, 5 - formattedNews.length)).fill({
          title: "Coming Soon...",
          date: "--",
          _id: null,
        }),
  ...
  const fetcDoctorAdvices = async () => {
    try {
      ...
      const filledDA = [
        ...formattedDA,
        ...Array(Math.max(0, 5 - formattedDA.length)).fill({
          title: "Coming Soon...",
          date: "--",
          _id: null,
        }),
    ...
    <div className="flex flex-col md:flex-row justify-center gap-8 px-6 md:px-28">
      ...
      >
        <div className="bg-[#34d399] text-center text-white p-6">
          ...
          <h3 className="text-lg font-semibold tracking-wide">DOCTOR ADVICE</h3>
        </div>

        <ul className="p-4 space-y-3">
          {doctorAdviceList.map((item, i) => {
            const isPlaceholder = item.title === "Coming Soon...";
            return (
              <li
                key={i}
